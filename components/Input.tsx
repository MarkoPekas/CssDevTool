...
const cssStyles = [
    ...
    'background-clip ', 'background-color', 'background-image', 'background-origin ', 'background-position', 'background-repeat', 'background-size ', 'border', 'border-bottom', 
    ...
    'border-image ', 'border-image-outset ', 'border-image-repeat ', 'border-image-slice ', 'border-image-source ', 'border-image-width ', 'border-left', 'border-left-color', 
    ...
    'bottom', 'box-shadow ', 'box-sizing ', 'caption-side', 'clear', 'clip', 'color', 'column-count ', 'column-fill ', 'column-gap ', 'column-rule ', 'column-rule-color ', 
    'column-rule-style ', 'column-rule-width ', 'column-span ', 'column-width ', 'columns ', 'content', 'counter-increment', 'counter-reset', 'cursor', 'direction', 
    'display', 'empty-cells', 'flex ', 'flex-basis ', 'flex-direction ', 'flex-flow ', 'flex-grow ', 'flex-shrink ', 'flex-wrap ', 'float', 'font', 'font-family', 
    'font-size', 'font-size-adjust ', 'font-stretch ', 'font-style', 'font-variant', 'font-weight', 'height', 'justify-content ', 'left', 'letter-spacing', 'line-height', 
    'list-style', 'list-style-image', 'list-style-position', 'list-style-type', 'margin', 'margin-bottom', 'margin-left', 'margin-right', 'margin-top', 'max-height', 
    'max-width', 'min-height', 'min-width', 'opacity ', 'order ', 'outline', 'outline-color', 'outline-offset ', 'outline-style', 'outline-width', 'overflow', 'overflow-x', 
    'overflow-y ', 'padding', 'padding-bottom', 'padding-left', 'padding-right', 'padding-top', 'page-break-after', 'page-break-before', 'page-break-inside', 'perspective', 
    ...
    'transform-style ', 'transition ', 'transition-delay ', 'transition-duration ', 'transition-property ', 'transition-timing-function ', 'vertical-align', 'visibility', 
    'white-space', 'width', 'word-break ', 'word-spacing', 'word-wrap ', 'z-index']

const Input = (props: {val:boolean, value: string, key: string, suggestions: string[], onChange: (value: string) => void, checked: boolean, newline: (id: string) => void}) => {
    const [focused, setFocused] = createSignal(false);
    const [value, setValue] = createSignal(props.value || '');
    const [suggestions, setSuggestions] = createSignal(props.suggestions);
    const [selected, setSelected] = createSignal(0);
    const [escape, setEscape] = createSignal(false);
    ...
    return (
        <div style={{position: 'relative'}}>
            ...
            onKeyDown={(e) => {
                ...
                if(e.key === 'Enter') {
                    ...
                    if(props.val){
                        ...
                        let s = random.split('-')
                        let el = document.getElementById(`input-val-${s[0]}-${parseInt(s[1])+1}`)
                        if(el)
                            el.focus()
                        else
                            props.newline(`input-val-${s[0]}-${parseInt(s[1])+1}`)
                    }
                    e.preventDefault();
                }
                if(e.key === 'Tab') {
                    document.getElementById(`input-${props.val?'val':'prop'}-${random}`)?.blur();
                    if(props.val){
                        let el = document.getElementById(`input-prop-${random}`)
                        if(el)
                            el.focus()
                        else
                            props.newline(`input-prop-${random}`)
                    }else{
                        let s = random.split('-')
                        let el = document.getElementById(`input-val-${s[0]}-${parseInt(s[1])+1}`)
                        if(el)
            ...
            <div style={{position: 'absolute', "z-index": 10, width: '250px', 'text-align': 'left', "max-height": '400px', "overflow-y": 'auto', display: (focused()&&suggestions().length>0)?'':'none'}}>
